...
// evaluates these. Ben's own workbook already has real, worked-out logic
// here (his Combo Rules tab), so rather than invent placeholders, these
// are transcribed from that tab (in our own words, structured so code can
// evaluate them, not copied verbatim) as real starting content.
//
...
export const COMBO_RULES: ComboRule[] = [
  ...
  },
  {
    ...
    recommendedAction: "A transaction would stall on day one. Position F&W as the confidential outside deal team, and start delegating operational duties in parallel.",
  },
  {
    id: "R7",
    conditions: [
