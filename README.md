...
rated 1-5 with its own specific rubric, and gets a scored readiness report
immediately. Every submission is saved and reviewable by F&W as a table
and a downloadable Excel export.

This rebuild fixes the one thing Ben flagged about the old prototype: the
1-5 scale used to mean the same generic "strongly disagree to strongly
agree" on every question. Now every question defines its own 5-point
rubric (see `lib/questions.ts`), so a 3 on one question and a 3 on another
represent two different, specific, real states of the business.
...
- **A passcode, not a full login system**, protects `/admin` (checked in
  ...
  convention). The assessment itself has no login, per Noah: it's going
  to Ben first and isn't handling anything regulated yet.

### The data model that matters most: `lib/questions.ts`

Every one of the 28 questions defines its own five rating levels, each
with a `tier` (Poor/Fair/Good/Excellent, matching Ben's existing
vocabulary from his Excel workbook), a short `label` (shown under each
...
Ben's Excel workbook has real infrastructure beyond a scorecard: an
...
(the Combo Rules are transcribed from Ben's actual workbook; the Action
Library is a first-pass draft for all 28 questions, in his voice, meant
to be reacted to and edited exactly like his own workbook already says
...
Because deploys are automatic on push, once that's built, Ben sees it by
refreshing the same link, no new link, no re-explaining, no one needing
to drive a laptop over to demo a localhost.
