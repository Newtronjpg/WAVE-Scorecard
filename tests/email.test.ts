...
describe("normalizeEmail", () => {
  it("lowercases mixed-case addresses", () => {
    ...
    // "NoahNewton2006@gmail.com" as not matching the account's address,
    // even though Gmail itself treats it identically to
    // "noahnewton2006@gmail.com". Resend's check is apparently an exact
    // string match, not case-insensitive, so this has to be normalized
    // before it ever reaches them.
    expect(normalizeEmail("NoahNewton2006@gmail.com")).toBe("noahnewton2006@gmail.com");
  });

  it("trims surrounding whitespace", () => {
    expect(normalizeEmail("  ben@fw-cpa.com  ")).toBe("ben@fw-cpa.com");
  });

  it("is a no-op on an already-normalized address", () => {
    expect(normalizeEmail("sarah@fw-cpa.com")).toBe("sarah@fw-cpa.com");
  });
});
